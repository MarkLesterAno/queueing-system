...
import { getEstimatedWait, type Ticket } from "@/lib/queue"
      ...
      >
        ...
        >
          ...
          <p className="text-xs text-muted-foreground text-center font-mono leading-relaxed max-w-xs">
            Please proceed to the {office.name} waiting area. Your number will
            be displayed when called.
          </p>
...
export default function CenralKiosk() {

  const [selectedOffice, setSelectedOffice] = useState<IOfficeStat | null>(null);
  ...
  const { data, isLoading, mutate } = useAllOffices(1500);
  ...
  const handleSelectOffice = async (office: IOfficeStat) => {
          ...
          {offices.map((office) => {
            return (
              <OfficeCard
                key={office.id}
                office={office}
                queueDepth={office.queueDepth}
                estWait={getEstimatedWait(office?.queueDepth ?? 0)}
